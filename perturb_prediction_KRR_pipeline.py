"""
Perturb-Prediction KRR Pipeline
------------------------------------------
This script implements a Kernel Ridge Regression (KRR) baseline for predicting transcriptional
...
1. Multiple Kernel Aggregation (MKL): Combines knowledge priors (ESM, GenePT, pathways),
   cross-context mean responses, and local PCA loadings into a consensus kernel matrix.
...
class DataModule:
    ...
    def load_and_preprocess(self, 
        ...
        # List to hold kernels from each source (Shape: n_unique_perts x n_unique_perts)
        ...
        if use_data_prior:
            ...
            # Each other context acts as one kernel source
            ...
            for context in other_contexts:
                ...
                # Compute means for this context
                pert_means = {}
                ...
                # Create Feature Matrix for this context (NaN if pert not in context)
                ...
                if valid_mask.any():
                    # Store the raw feature matrix (X_c) for later kernel computation
        ...
        # 5. Aggregation: Handled dynamically in optimize_and_train loop
...
def run_kfold_cv(dm: DataModule, 
    ...
    # Kernel indexing is based on unique perturbations
...
def aggregate_kernels(dm: DataModule, kernel_type: str = 'linear', gamma: Optional[float] = None):
    """Generates the aggregated kernel from multiple sources with specified params using memory-efficient online aggregation."""
    import gc
    ...
    for X, name in dm.kernel_sources:
        ...
        if len(valid_indices) > 0:
            ...
            if kernel_type == 'linear':
                ...
                # For RBF, normalization is less critical but can be done
                norms = np.linalg.norm(X_valid, axis=1, keepdims=True)
                X_valid = X_valid / (norms + 1e-9)
                from sklearn.metrics.pairwise import rbf_kernel
                k_vals = rbf_kernel(X_valid, gamma=gamma).astype(np.float32)
...
def optimize_and_train(h5ad_path: str,
    ...
    for i, config in enumerate(param_grid):
        # Only re-aggregate if kernel params change
        current_kernel_params = (config.get('kernel_type', 'linear'), config.get('gamma', None))
        if current_kernel_params != last_kernel_params:
            dm.aggregated_kernel = aggregate_kernels(dm, kernel_type=current_kernel_params[0], gamma=current_kernel_params[1])
    ...
    dm.aggregated_kernel = aggregate_kernels(dm, kernel_type=best_config.get('kernel_type', 'linear'), gamma=best_config.get('gamma', None))
    ...
    output_filename = f"./baseline_results/KRR_predictions__{split_column}__MSE__{tag_p}.h5ad"
