...
class Model(object):
    def __init__(self, 
             ...
             multi_gene = True,
             use_tg_coord = False,
             ctrl_size = None, # reduce the number of control cells used for evaluation if there are too many control cells
             pert_delta_context_col = None,   
             pert_delta_target_context = None, 
             ):
        ...
        # control cells (POOLED — across every context present in adata)
        ...
        self.ctrl_mean = np.mean(ctrl_x, axis=0)   # POOLED mean — used for ALL centering, everywhere, unchanged
        ...
        #val_ctrl_x- control cells used specifically by train()'s
        #internal validation-scoring check. Restricted to the target context (if specified) 
        #so predicted val-gene effects are generated from genuine target-context basal cells
        self.pert_delta_context_col = pert_delta_context_col
        self.pert_delta_target_context = pert_delta_target_context

        if pert_delta_context_col is not None and pert_delta_target_context is not None:
            val_ctrl_mask = (adata.obs[pert_delta_context_col].values == pert_delta_target_context)
            if 'ctrl' in np.unique(adata.obs['condition'].values):
                val_ctrl_mask = val_ctrl_mask & (adata.obs['condition'].values == 'ctrl')
            val_ctrl_x_raw = adata[val_ctrl_mask].X
            self.val_ctrl_x = torch.from_numpy(
                val_ctrl_x_raw - self.ctrl_mean.reshape(1, -1)   #POOLED mean, same as self.ctrl_x
            ).float().to(self.device)
        else:
            self.val_ctrl_x = self.ctrl_x  #fallback
        ...
        if pert_delta_context_col is not None and pert_delta_target_context is not None:
            context_mask = (self.adata.obs[pert_delta_context_col].values == pert_delta_target_context)
        else:
            context_mask = np.ones(self.adata.shape[0], dtype=bool)  # original pooled behavior, unchanged

        self.pert_delta = {}
        for i in np.unique(self.adata.obs['condition'].values):
            mask_i = (self.adata.obs['condition'].values == i) & context_mask
            if mask_i.sum() == 0:
                # this gene has no cells in the target context — skip rather than
                # silently computing a delta from other contexts
                continue
            adata_i = self.adata[mask_i]
        ...
        if retrain:
            if len(self.pert_val) > 0: # If validating
                ...
                for i in self.pert_val:
                    ...
                    # uses val_ctrl_x (target-context control cells if configured)
                    val_p = torch.from_numpy(np.tile(pert_emb_p, 
                                                     (self.val_ctrl_x.shape[0], 1))).float().to(self.device)
                    genes = i.split('+')
                    tg_loc_i = gene2loc(genes, self.adata.var.gene_name.values, self.n_tgs)
                    tg_loc = torch.from_numpy(np.tile(np.array(tg_loc_i), 
                                                     (self.val_ctrl_x.shape[0], 1))).long().to(self.device)
                    if self.use_tg_coord:
                        p_tg = [torch.from_numpy(np.tile(p_tg_i, 
                                                     (self.val_ctrl_x.shape[0], 1))).float().to(self.device) for p_tg_i in p_tg_ls]
                        x_hat, x_hat_tg_ls, p_hat, mean_z, log_var_z, s = self.Net(self.val_ctrl_x, val_p, tg_loc.long(), p_tg)
                        x_hat = adjust_tg(x_hat, x_hat_tg_ls, tg_loc, self.ctrl_mean_tensor)
                    else:
                        x_hat, p_hat, mean_z, log_var_z, s = self.Net(self.val_ctrl_x, val_p)
                    x_hat = np.mean(x_hat.detach().cpu().numpy(), axis=0)
