#!/bin/bash
# Parallel GPU dispatch for presage final training sweep (only variant, K562GW, rs swept 1..3)
set -u
shopt -s nullglob

# ---- config ----
NUM_GPUS="${NUM_GPUS:-$(nvidia-smi --query-gpu=count --format=csv,noheader | head -n1 | tr -d ' ')}"
LOG_DIR="${LOG_DIR:-./logs_final/K562GW}"
mkdir -p "$LOG_DIR"
...
# ---- build job list ----
JOBS=()
for file in ./search_spaces_only/K562GW/HPO_only_*.json; do
    base="${file##*/}"
...
if [[ ${#JOBS[@]} -eq 0 ]]; then
    echo "No matching JSON files found in ./search_spaces_only/K562GW/"
    exit 1
fi
...
run_job() {
    ...
    local log="${LOG_DIR}/only_${name}.log"

    echo "[GPU ${gpu_id}] START  ${name}"
    CUDA_VISIBLE_DEVICES="$gpu_id" python presage_pbulk_train_final.py \
        --stage final \
        --variant only \
        --prepared_dir "../data/final_rs${rs}/hpo_only_${cell}_${cell}_TF_${tf}_UF_10_rs_${rs}_${mode}" \
        --ds_config_file ../configs/dataset_base_config_hpo_singlecont_V3.json \
        --hpo_num_folds 1 \
        --split_name "$name" \
