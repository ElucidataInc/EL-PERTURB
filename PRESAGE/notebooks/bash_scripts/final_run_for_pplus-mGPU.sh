#!/bin/bash
# Parallel GPU dispatch for presage final training sweep (pplus variant, K562GW, rs swept 1..3)
set -u
shopt -s nullglob
...
# ---- build job list ----
JOBS=()
for file in ./search_spaces_pplus/K562GW/HPO_pplus_*.json; do
    base="${file##*/}"
...
if [[ ${#JOBS[@]} -eq 0 ]]; then
    echo "No matching JSON files found in ./search_spaces_pplus/K562GW/"
    exit 1
fi
...
run_job() {
    ...
    CUDA_VISIBLE_DEVICES="$gpu_id" python presage_pbulk_train_final.py \
        --stage final \
        --variant pplus \
        --prepared_dir "../data/final_rs${rs}/hpo_pplus_${cell}_${cell}_TF_${tf}_UF_10_rs_${rs}_${mode}" \
