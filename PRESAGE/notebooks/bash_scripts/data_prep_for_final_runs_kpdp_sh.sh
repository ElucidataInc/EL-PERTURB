...
run_job() {
    ...
    local split="${cline}_TF_${TF}_UF_10_rs_${seed}_${stype}"
    local target="$cline"

    echo "[START] kpdp | target=$target | TF=$TF | split=$stype| seed=$seed"
    ...
    python prepare_presage_data_memopt_scdata.py \
        ...
        --n_nmf_embedding "128" \
        --output_dir "../data/final_rs${seed}/hpo_kpdp_${target}_${split}" \
        --stage "final" \
        --variant "kpdp" \
        --skip_degs
